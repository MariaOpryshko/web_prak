# Схема навигации
![](schemas/Схема_проекта.jpg)
## Выбор списка
- ссылка на список всех проектов
- ссылка на список сотрудников
- ссылка на список политик выплат

## Список проектов
- ссылка на страницу, позволяющую добавить новый проект
- ссылка на личную страницу проекта с помощью поиска по имени проекта или его номера
- список проектов содержит названия проектов и их номера. Оба поля являются ссылками на личную страницу проекта

## Список политик выплат
- список политик выплат содержит названия политик и сумму их выплат
- ссылка на страницу, позволяющую добавить новую политику выплат
- кнопка "Редактировать"; после нажатия дается возможность изменить сумму выплат, а так же статус всех политик на странице (состояние страницы после нажатия кнопки "Редактировать" назовем режимом редактирования)
- в режиме редактирования напротив каждой политики появляеься кнопка "Удалить"
- в режиме редактирования появляется кнопка "Подтвердить", которая сохрает все изменения и подтверждает удаление политик

## Список людей
- ссылка на страницу, позволяющую добавить нового сотрудника
- ссылка на личную страницу сотрудника с помощью поиска по его имени, личному номеру, должности и т.д.
- возможность фильтрации сотрудников по должности
- список сотрудников содержит личный номер сотрудника, его имя и должность. Имя сотрудника и его номер являются ссылками на личную страницу сотрудника

## Форма добавления проекта
- страница просит указать данные проекта: его название, даты начала и конца, его статус (активен/не активен)
- переход на список проектов (и добавление нового проекта в этот список)

## Форма добавления политики выплат
- страница просит указать тип политики (по роли в проекте, по должности, по особому случаю)
- область ввода необходимых данных исходя из выбора типа новой политики
- переход на список политик (и добавление новой политики в этот список)

## Форма добавления нового сотрудника
- страница просит указать личные данные сотрудника, а так же дает выбрать ему должность исходя из существующих политик выплат "по должности"
- переход на список сотрудников (и добавление нового сотрудника в этот список)

## Личная страница проекта
- содержит данные проекта: название, статус проекта(активен/не активен), дата начала проекта(и его конца, если проект уже завершен или имеет срок выполнеия)
- список участников проекта: их личные номера, имена, роли в проекте, дата начала работы над проектом(и дата конца работы над проектом, если участник был переведен на другую роль, уволен или работает на проекте по контракту определенное время), а также статус данного участника(активен/ не активен, в зависимости от того актуальна ли роль участинка или его участие в проекте на настоящий момент времени). Личный номер участников проекта и их имена являются ссылками на личную страницу данного участника
- кнопка "Редактировать проект", которая переводит страницу в режим редактирования и позволяет поменять название проекта, его статус и дату окончания; также появляется кнопка "Удалить проект", которая позволяет полностью удалить проект из базы данных, если он был создан случайно
- в режиме редактирования так же пояляется кнопка "Подтвердить", которая подтверждает изменение данных о проекте и переводит страницу в обычный режим
- кнопка "Редактировать участников", которая является ссылкой на страницу редактирования участников проекта

## Личная страница сотрудника
- актуальные данные сотрудника: все личные данные, актуальная должность, проект, роль в проекте, заработная плата за последний месяц работы. Проект является ссылкой на личную страницу проекта
- ссылка на историю проектов, в которых принимал участие работник
- ссылка на историю всех выплат сотруднику
- ссылка на историю должностей сотрудника
- кнопка "Редактировать", которая является ссылкой на страницу редактирования страницы сотрудника 

## Редактирование участников проекта
- содержит данные об участниках проекта: их личные номера, имена, роли в проекте, даты начала и конца работы над проектом, а также статус данного участника(активен/ не активен)
- напротив каждого участника есть кнопка "Редактировать", которая позволяет изменить статус участника и дату окончания его работы (после нажатия кнопки "Редактировать" появляется кнопка "Подтвердить", которая поддтверждает изменение данной строки списка)
- напротив каждого участника есть кнопка "Удалить", которая позволяет полностью удалить участника из проекта, если он был добавлен случайно
- кнопка "Добавить участника", которая является ссылкой на страницу создания нового участника проекта

## Создание участника проекта
- поле, которое позволяет выбрать сотрудника, которого необходимо назначить, с помощью полнотекстового поиска среди списка сотрудников
- выпадающий список, который позволяет выбрать роль в проекте (составлен исходя из существующих активных политик выплат)
- поле для заполнения даты, с которой и по которую человек будет занимать данную роль
- кнопка "Подтвердить" (по сути кнопка формирует строку в таблице "Назначение выплаты" и "Назначение на проект")
  
## Редактирование страницы сотрудника
- содержит личные данные сотрудника(ФИО, домашний адрес и т.д.) и его должность
- разрешается изменить все личные данные сотрудника
- кнопка "Назначить на должность", которая дает выбрать из выпадающего списка должность, а так же просит заполнить дату, с которой человек будет занимать данную должность ( по сути данная кнопка формирует строку в таблице "Назначение выплаты", так как должности из выпадающего списка являются теми должностями, для которых существует активная политика выплат)
- кнопка "Удалить", которая позволяет полностью удалить сотрудника из базы данных
- кнопка "Подтвердить", которая сохраняет изменения в личных данных сотрудника и его должности

## История проектов сотрудника
- страница содержит все проекты сотрудника, его роли на этих проектах и даты, когда сотрудник занимал такой статус
- названия проектов являются ссылками на личные страницы этих проектов

## История выплат сотрудника
- страница содержит все данные о выплатах сотруднику: политики выплат, сумму начислений и даты, когда были произведены начисления

## История должностей сотрудника
- страница содержит все должности сотрудника и даты, когда сотрудник занимал их

# Стратегии использования
## Получение карьерной истории/истории выплат/истории проектов
1) выбор списка сотрудников на первой странице
2) поиск сотрудника по имени/личному номеру и переход на его личную страницу
3) выбор кнопки "История должностей"/"История выплат"/"История проектов" и переход на необходимую страницу

## Назначение служащего на новую должность
1) выбор списка сотрудников на первой странице
2) поиск сотрудника по имени/личному номеру и переход на его личную страницу
3) нажать кнопку "Редактировать"
4) нажать кнопку "Назначить на должность"
5) из выпадающего списка возможных должностей выбрать новую должность, а также указать дату начала работы на должности
6) нажать кнопку "Подтвердить"

## Добавить служащего в проект
1) выбор списка проектов на первой странице
2) поиск проекта по названию и переход на его личную страницу
3) нажать кнопку "Редактировать участников"
4) нажать кнопку "Добавить участника"
5) ввести данные о новом участнике
6) нажать кнопку "Подтвердить"

## Удалить служащего из проекта
1) выбор списка проектов на первой странице
2) поиск проекта по названию и переход на его личную страницу
3) нажать кнопку "Редактировать участников"
4) нажать кнопку "Удалить" напротив служащего

## Добавление служащего/проект/политику выплат
1) выбор списка сотрудников/проектов/политик выплат на первой странице
2) нажать кнопку "Добавить сотрудника"/"Добавить проект"/"Добавить политику выплат"
3) в появившейся форме заполнить данные о служащем/проекте/политики выплат
4) нажать кнопку "Подтвердить"

## Удаление служащего
1) выбор списка сотрудников на первой странице
2) поиск сотрудника по имени/личному номеру и переход на его личную страницу
3) нажать кнопку "Редактировать"
4) нажать кнопку "Удалить"

## Удаление проекта
1) выбор списка проектов на первой странице
2) поиск проекта по имени/личному номеру и переход на его личную страницу
3) нажать кнопку "Редактировать проект"
4) нажать кнопку "Удалить проект"

## Удаление политики выплат
1) выбор списка политик выплат на первой странице
3) нажать кнопку "Редактировать"
4) нажать кнопку "Удалить" напротив необходимой политики
5) нажать кнопку "Подтвердить"

# Схема базы данных
![](schemas/Схема%20бд.jpg)
